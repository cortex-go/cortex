...
function renderFeed(){const s=active(),box=$('#feed');const wasNear=nearBottom(box);box.innerHTML='';if(!s.events.length){box.innerHTML='<div class="empty"><span class="orb">C</span><strong>What do you want to build?</strong><span>Choose a workspace, then give Cortex a development task.</span></div>';$('#newActivity').hidden=true;return}for(const ev of s.events){if(ev.kind==='task')continue;box.append(eventNode(ev))}if(wasNear){box.scrollTop=box.scrollHeight;s.followBottom=true;$('#newActivity').hidden=true}else if(s.followBottom){box.scrollTop=box.scrollHeight}renderTaskPanel(s);updateNewActivity(s)}// sessionSafe persists run state and current run id for the tab indicator.
function renderTaskPanel(s){const panel=$('#taskPanel');if(!panel)return;const list=$('#taskList');const latest=[...s.events].reverse().find(e=>e.kind==='task');if(!latest){list.innerHTML='';panel.hidden=true;return}let todos=[];try{todos=JSON.parse(latest.text||'[]')}catch{todos=null}if(!Array.isArray(todos)){panel.hidden=true;return}if(!todos.length){list.innerHTML='';$('#taskProgress').textContent='0 of 0 completed';panel.hidden=true;return}panel.hidden=false;list.innerHTML='';const done=todos.filter(t=>t.status==='completed').length;const order={in_progress:0,pending:1,completed:2};const sorted=[...todos].sort((a,b)=>(order[a.status]??3)-(order[b.status]??3));for(const t of sorted){const row=document.createElement('div');row.className='task-row '+t.status;const mark=document.createElement('span');mark.className='task-mark';mark.textContent=t.status==='completed'?'✓':t.status==='in_progress'?'●':'·';mark.setAttribute('aria-hidden','true');const label=document.createElement('span');label.className='task-label';label.textContent=t.content;row.append(mark,label);list.append(row)}$('#taskProgress').textContent=`${done} of ${todos.length} completed`}
